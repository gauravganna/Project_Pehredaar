...
before=0
sleep 10
while  [ $x -le 2 ]													#Will mark entries for total of 2 minutes at the start time of lecture.
do
...
before=0
sleep 10
while  [ $x -le 2 ]											#Will mark entries for total of 2 minutes at the end time of lecture.
do
	echo "Loop Number "$x""
  	NEW=`ls -l Attendance_Sheet.txt`							#Data to determine whether the file has changed from the previous time snapshot.
  	if [ "$NEW" != "$LAST" ]; then								#If file has changed.
